  ...
  class BaseRecords
  {
    ...
    public static function get_all()
    {
      try {
        $class = get_called_class();
        $r = [];
        $sql = "Select * from `".$class::get_table()."`  C";
    ...
    public static function get_all_pagination($page, $limit=null)
    {
      try {
        $limit = $limit == null ? \siteinfo("pagelimit") : $limit;
        $offset        = ($page-1)*$limit;
        $class = get_called_class();
        $r = [];
        $sql = "SELECT * FROM `".$class::get_table()."` LIMIT ? OFFSET ?";
    ...
    public static function get_or_all_by($k, $v, $k2, $v2)
    {
      try {
        $class = get_called_class();
        $r = [];
        $sql = "Select * from `".$class::get_table()."` WHERE `{$k}` IN (?) OR `{$k2}` IN (?)";
  			$result = Self::link()->FetchAllObject($sql, [$v,$v2]);
    ...
    public static function get_all_by($k, $v)
    {
      try {
        $class = get_called_class();
        $r = [];
        $sql = "Select * from `".$class::get_table()."` WHERE `{$k}` IN (?)";
