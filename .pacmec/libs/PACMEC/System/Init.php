...
namespace PACMEC\System
{
  class Init
  {
     ...
     * RUN Session
     */
    public static function run_session()
    {
      if(Self::check_session() === FALSE || \session_id() === "") \session_start();
    }

    public function __construct()
    {
      if($GLOBALS['PACMEC']['route']->theme == null) $GLOBALS['PACMEC']['route']->theme = \siteinfo('theme');
      $this->layout = $GLOBALS['PACMEC']['route']->theme;

      \add_action('page_title', function(){ if(isset($GLOBALS['PACMEC']['route']->id)){ echo (\pageinfo('page_title') !== 'NaN') ? \__at(\pageinfo('page_title')) : \__at(\pageinfo('title')); } });
      \add_action('page_description', function(){ if(isset($GLOBALS['PACMEC']['route']->id)){ echo (\pageinfo('page_description') !== 'NaN') ? \pageinfo('page_description') : \__at(\pageinfo('description')); } });
      \add_action('page_body', function(){
        if(isset($GLOBALS['PACMEC']['route']->path) && $GLOBALS['PACMEC']['route']->path !== ""){
      ...
      $acti = \activation_theme($GLOBALS['PACMEC']['site']->theme);
      \do_action('init');

      if(!isset($GLOBALS['PACMEC']['theme']))
      {
        #echo $GLOBALS['PACMEC']['route']->theme;
        $acti = \activation_theme($GLOBALS['PACMEC']['site']->theme);
      }
      if(isset($GLOBALS['PACMEC']['theme']['path']) && is_file($GLOBALS['PACMEC']['theme']['path'] . '/index.php') && file_exists($GLOBALS['PACMEC']['theme']['path'] . '/index.php'))
      {
        $data = $GLOBALS['PACMEC']['input'];
        if(isset($data['controller']))
        {
          $controllerObj = \cargarControlador($data["controller"]);
          \lanzarAccion($controllerObj);
        }
        else require_once $GLOBALS['PACMEC']['theme']['path'] . '/index.php';
      } else {
    ...
    */
    ...
    {
      \add_style_head(\siteinfo('url')   . "/.pacmec/assets/css/pacmec.css",               ["rel"=>"stylesheet", "type"=>"text/css", "charset"=>"UTF-8"], 1, false);
      \add_style_head(\siteinfo('url')   . "/.pacmec/assets/css/plugins.css",              ["rel"=>"stylesheet", "type"=>"text/css", "charset"=>"UTF-8"], 0.99, false);

      \add_scripts_head(\siteinfo('url') . "/.pacmec/assets/js/sweetalert2.all.min.js",    ["type"=>"text/javascript", "charset"=>"UTF-8"], 0.99, false);
      #add_scripts_head(\siteinfo('url') . "/.pacmec/assets/js/pacmec-scripts-top.js",     ["type"=>"text/javascript", "charset"=>"UTF-8"], 1, false);
      ### \add_scripts_head(\siteinfo('url') . "/.pacmec/assets/dist/jquery/jquery.min.js",   ["type"=>"text/javascript", "charset"=>"UTF-8"], 1, false);
      \add_scripts_head(\siteinfo('url') . "/.pacmec/assets/js/axios.js",                  ["type"=>"text/javascript", "charset"=>"UTF-8"], 0.90, false);
      \add_scripts_head(\siteinfo('url') . "/.pacmec/assets/js/vue.min.js",                ["type"=>"text/javascript", "charset"=>"UTF-8"], 0.89, false);
      \add_scripts_head(\siteinfo('url') . "/.pacmec/assets/js/vue-router.js",             ["type"=>"text/javascript", "charset"=>"UTF-8"], 0.88, false);
    ...
    public static function initMetas()
    {
      ...
      \pacmec_add_meta_tag('language', $GLOBALS['PACMEC']['lang-detect']);
